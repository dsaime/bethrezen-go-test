...
func TestRepository(t *testing.T, newRepository func() newsAgr.Repository) {
	t.Run("List", func(t *testing.T) {
		t.Run("из пустого репозитория вернется пустой список", func(t *testing.T) {
			r := newRepository()
			chats, err := r.List(newsAgr.Filter{})
		...
		t.Run("без фильтра из репозитория вернутся все сохраненные чаты", func(t *testing.T) {
			r := newRepository()
			chats := make([]newsAgr.Chat, 10)
			for i := range chats {
				chats[i] = upsertChat(t, r, rndChat(t))
			}
			chatsFromRepo, err := r.List(newsAgr.Filter{})
			assert.NoError(t, err)
...
func rndChat(t *testing.T) newsAgr.Chat {
...
func upsertChat(t *testing.T, r newsAgr.Repository, chat newsAgr.Chat) newsAgr.Chat {
	err := r.Upsert(chat)
	require.NoError(t, err)
...
func rndParticipant(t *testing.T) newsAgr.Participant {
...
func addRndParticipant(t *testing.T, chat *newsAgr.Chat) {
	p, err := newsAgr.NewParticipant(uuid.New())
	require.NoError(t, err)
...
func addRndInv(t *testing.T, chat *newsAgr.Chat) {
	inv, err := newsAgr.NewInvitation(common.RndElem(chat.Participants).UserID, uuid.New())
	require.NoError(t, err)
