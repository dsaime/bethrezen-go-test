...
import (
	...
	mockOauth "newsapi/internal/usecases/users/oauth/mocks"
)
...
func (suite *Suite) UpsertChat(chat newsAgr.Chat) newsAgr.Chat {
...
func (suite *Suite) RndChat() newsAgr.Chat {
	chat, err := newsAgr.NewChat(gofakeit.Noun(), uuid.New(), nil)
	suite.Require().NoError(err)
...
func (suite *Suite) NewParticipant(userID uuid.UUID) newsAgr.Participant {
	p, err := newsAgr.NewParticipant(userID)
	suite.Require().NoError(err)
	return p
}

// AddRndParticipant добавляет случайного участника в чат
func (suite *Suite) AddRndParticipant(chat *newsAgr.Chat) newsAgr.Participant {
	p, err := newsAgr.NewParticipant(uuid.New())
	suite.Require().NoError(err)
...
func (suite *Suite) AddParticipant(chat *newsAgr.Chat, p newsAgr.Participant) {
	suite.Require().NoError(chat.AddParticipant(p, nil))
}

// NewInvitation создает новое приглашение
func (suite *Suite) NewInvitation(subjectID, recipientID uuid.UUID) newsAgr.Invitation {
	i, err := newsAgr.NewInvitation(subjectID, recipientID)
	suite.Require().NoError(err)
	return i
}

// AddInvitation добавляет приглашение в чат
func (suite *Suite) AddInvitation(chat *newsAgr.Chat, i newsAgr.Invitation) {
...
func (suite *Suite) RandomOauthUser() userr.OpenAuthUser {
	u, err := userr.NewOpenAuthUser(
		...
		RandomString(6)+" "+RandomString(7),                   // Title
